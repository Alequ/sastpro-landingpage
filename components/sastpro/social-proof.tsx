...
export function SocialProof() {
  ...
  const testimonials = [
    ...
    {
      quote:
        "The electrical asset tracking feature alone saved us thousands in preventive maintenance costs.",
    ...
    {
      quote:
        "Audit preparation used to take weeks. Now we're audit-ready 24/7 with just a few clicks.",
        ...
        <div className="mx-auto mt-16 grid max-w-6xl grid-cols-1 gap-8 lg:grid-cols-3">
          {testimonials.map((testimonial, index) => (
            ...
            >
              ...
              <div
                className={`absolute inset-0 ${testimonial.bgGlow} opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-2xl`}
              />
              ...
              <div className="relative p-6">
                ...
                {hoveredIndex === index && (
                  <div
                    className={`mt-4 inline-flex items-center gap-2 rounded-lg bg-gradient-to-r ${testimonial.color} px-3 py-2 animate-in fade-in slide-in-from-bottom duration-300`}
                  >
                ...
                <div className="mt-6 flex items-center gap-3">
                  {/* Avatar */}
                  <div
                    className={`flex h-12 w-12 items-center justify-center rounded-full bg-gradient-to-br ${testimonial.color} shadow-lg group-hover:scale-110 transition-transform duration-300`}
                  >
                ...
                {/* Bottom accent line */}
                <div
                  className={`mt-6 h-1 rounded-full bg-gradient-to-r ${testimonial.color} opacity-0 group-hover:opacity-100 transition-all duration-500 transform origin-left scale-x-0 group-hover:scale-x-100`}
                />
              </div>
            </div>
          ))}
        ...
        <div className="mx-auto mt-20 max-w-5xl">
          ...
          <div className="grid grid-cols-2 gap-6 sm:grid-cols-3 lg:grid-cols-6">
            {companies.map((company, index) => (
              ...
              >
                <span
                  className={`text-xs font-semibold text-center transition-colors ${company.color} opacity-70 group-hover:opacity-100`}
                >
                  {company.name}
                </span>
              </div>
