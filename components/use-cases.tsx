...
import { HardHat, ClipboardCheck, Building2, Users, Target, TrendingUp, Quote, Star } from "lucide-react"
  ...
  const useCases = [
    {
      ...
      example: "Reduced incident reporting time by 75% and achieved 100% audit pass rate.",
    ...
    },
    {
      ...
      description: "Automate compliance tracking, manage certifications, and generate audit-ready reports instantly.",
      example: "Streamlined quarterly audits from 2 weeks to 2 days of preparation.",
    ...
    },
    {
      ...
      example: "Improved asset uptime by 40% with proactive maintenance scheduling.",
    ...
    },
    {
      ...
      example: "Enabled field teams to access critical documents 10x faster on mobile devices.",
      ...
      <div className="relative mx-auto max-w-7xl px-6 lg:px-8">
        ...
        <div className="mx-auto max-w-2xl text-center">
          ...
          <p className="mt-6 text-pretty text-lg leading-relaxed text-muted-foreground max-w-xl mx-auto">
            From the field to the office, SASTPRO empowers your entire team with role-specific features.
          </p>
        </div>
        ...
        <div className="mx-auto mt-16 grid max-w-6xl grid-cols-1 gap-8 sm:grid-cols-2">
          {useCases.map((useCase, index) => (
            ...
            >
              ...
              <div className={`absolute inset-0 ${useCase.bgGlow} opacity-0 group-hover:opacity-100 transition-opacity duration-500 blur-2xl`} />
              ...
              <div className="relative p-8">
                ...
                <div className="flex items-start justify-between mb-6">
                  {/* Icon */}
                  <div className={`flex h-14 w-14 items-center justify-center rounded-xl bg-gradient-to-br ${useCase.color} shadow-lg group-hover:scale-110 group-hover:rotate-6 transition-all duration-300`}>
                    <useCase.icon className="h-7 w-7 text-white" strokeWidth={2.5} />
                  </div>

                  {/* Stat badge */}
                  <div className={`rounded-lg border border-border/50 bg-background/50 px-3 py-2 backdrop-blur-sm transition-all duration-300 ${
                    hoveredIndex === index ? 'scale-110' : ''
                  }`}>
                    <div className={`text-2xl font-bold bg-gradient-to-r ${useCase.color} bg-clip-text text-transparent`}>
                      {useCase.stat}
                    </div>
                    <div className="text-xs text-muted-foreground whitespace-nowrap">{useCase.statLabel}</div>
                  </div>
                </div>
                ...
                <div className={`mt-6 relative rounded-xl bg-gradient-to-br from-secondary/50 to-secondary/30 p-4 border border-border/30 transition-all duration-300 ${
                  ...
                  <div className="relative">
                    ...
                    {hoveredIndex === index && (
                      <div className="mt-3 flex items-center gap-2 text-xs text-accent animate-in fade-in slide-in-from-left duration-300">
                        ...
                        <span className="font-medium">Real results from actual customers</span>
                      </div>
                    )}
                  </div>
                </div>
                ...
                {/* Bottom accent line */}
                <div className={`mt-6 h-1 rounded-full bg-gradient-to-r ${useCase.color} opacity-0 group-hover:opacity-100 transition-all duration-500 transform origin-left scale-x-0 group-hover:scale-x-100`} />
              </div>
            </div>
          ))}
        </div>
        ...
        <div className="mt-16 text-center">
          ...
          <div className="flex items-center justify-center gap-2">
            ...
            <span className="ml-2 text-sm font-medium text-foreground">4.9/5 from 200+ reviews</span>
          </div>
        </div>
      </div>
    </section>
